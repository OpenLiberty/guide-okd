...
The `mvn package` command will compile, verify, and build the project. The resulting compiled code will be packaged into a `tar.gz` compressed format which can be found at `system/target/system.tar.gz`. The package contains all the necessary files needed to run the microservice on an Open Liberty server, and is now ready to be injected into a Docker container for deployment.
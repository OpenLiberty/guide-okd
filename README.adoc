...
:page-related-guides: ['cloud-openshift', 'kubernetes-intro', 'containerize]
...
To interact with the OpenShift cluster, you will need to use the `oc` command. Minishift already includes the `oc` binary, to use the `oc` commands, run the 
following command to configure your PATH to include the binary:
...
//FIX Run this on the Windows machine and update with the appropriate command
...
The `mvn package` command will compile, verify, and build the project. The resulting compiled code will be packaged into a `tar.gz` archive which can be 
found at `system/target/system.tar.gz`. The archive contains all the configuration files, as well as the application needed to run the microservice on an 
...
Now that your Docker images are built, deploy them using a resource configuration file. Since OKD is built on top of Kubernetes, it supports the same 
...
The configuration file outlines a [hotspot=deployment file=0]`deployment` resource which will create and deploy a [hotspot=container file=0]`container` that 
will run the Docker-formatted image provided in the [hotspot=image file=0]`image` field. The [hotspot=image file=0]`image` field should point to your newly 
pushed image. 
...
Now you have to expose the service using a route so that external users can access the microservice through a host name, such as `www.mywebapp.com`. Update 
...
Access your microservice through the host name provided in the output, by visiting the following URL `http://[host-name]/system/properties`, or running the 
following command, ensuring to replace `[host-name]` with the proper host name provided by the `oc get routes` command.
...
[hotspot=inventoryImage file=0]`image` field with the appropriate image link.
...
[.tab_content.windows_section]
--
Substitute `[system-route-host-ip]` and `[inventory-route-host-ip]` with the appropriate values found by running the `oc get routes` command

[role=command]
```
mvn verify -Ddockerfile.skip=true \
-Dsystem.ip=[system-route-host-ip]  \
-Dinventory.ip=[inventory-route-host-ip]
```
--
[.tab_content.mac_section.linux_section]
--
...
To revert your Docker daemon back to its default settings, simply reboot your system.

...
You can use the same [hotspot file=1]`deploy.yaml` configuration file to deploy multiple microservices. Update the configuration file to include the deployment, service, and route resources for your `inventory` microservice.
...
----
[role="edit_command_text"]
Update the configuration file to add the [hotspot=inventoryResources file=1]`inventory` resources, making sure to update the inventory [hotspot=inventoryImage file=1]`image` field with the appropriate image link.

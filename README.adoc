...
You will learn how to deploy two simple microservices with Open Liberty to an Origin Community Distribution of Kubernetes (OKD) cluster running in Minishift.
...
functionality. OKD streamlines the DevOps process by providing an intuitive development pipeline. It also provides integration with multiple tools to make
the deployment and management of cloud applications easier. To learn more about the different platforms that Red Hat OpenShift offers, check out the 
...
The resulting output differs based on your OS and environment, but you will get an output similar to the following:
...
You can run through the development cycle using OpenShift's web console through the URL provided in the command output of the `minishift start` 
...
The web console provides a user friendly GUI alternative to their CLI tools that you can explore on your own. This guide will continue with the 
CLI tools.
...
found under the `system/target` directory. The archive contains the application needed to run the microservice on an Open Liberty server, and is now ready 
to be injected into a Docker container for deployment.
...
container named [hotspot=container file=0]`system-container` that will run the Docker-formatted image provided in the [hotspot=image file=0]`image` field. 
...
The system [hotspot=image file=0]`image` field specifies the name and tag of the container image you want to use for the system container. Update the 
...
The pod description includes an events log, which is useful in debugging any issues that may arise. The log will be formatted similar to the following:
...
so that you can make requests to your container. You also have to expose the service using a route so that external users can access the microservice through 
a host name. Update your [hotspot file=1]`deploy.yaml` file to include a service and route resource. 
...
----
[role="edit_command_text"]
Update the configuration file to include the [hotspot=systemService file=1]`service` and [hotspot=systemRoute file=1]`route` resource.
...
[hotspot=inventoryImage file=0]`image` field with the appropriate image link found in the `DOCKER REPO` column of the following command:
...
You will see a JSON response like the following. Note that your JSON response may not be formatted. The sample output has been formatted for readability:

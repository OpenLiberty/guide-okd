...
//file 0
deploy.yaml
[source, yaml, linenums, role="code_column hide_tags=systemRoute hide_tags=systemService"]
----
include::hotspots/deploy.yaml[]
----

Now that your Docker images are built, deploy them using a resource configuration file. 
...
The configuration file outlines a [hotspot=deployment file=0]`deployment` resource which will create and deploy a container that contains the Docker-formatted image given in the [hotspot=image file=0]`image` field.
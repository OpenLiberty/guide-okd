...
//file 0
pom.xml
[source, xml, linenums, role="code_column hide_tags=inventory"]
----
...
First, navigate to the `start` directory, where you will find the source code of the `system` and `inventory` microservice inside their respective directories. We will focus on the `system` microservice first, and will visit the `inventory` microservice later. Also included inside the `start` directory is the parent Maven Project Object Model (POM) file, the [hotspot file=0]`pom.xml`. To learn more about how to use Maven with Open Liberty, check out the https://openliberty.io/guides/maven-intro.html[Building a web application with Maven] guide. 
...
Now that the environment is set up, ensure that you are in the `start` directory and run the following command to build a Docker image according to the instructions in the newly created [hotspot file=1]`Dockerfile`:
...
The command builds an image named `system` from the [hotspot file=1]`Dockerfile` found in the `system` directory. To verify that the images are built, run the following command to list all local Docker images:
...
----
include::hotspots/SystemApplication.java[]
----
...
[source, java, linenums, role="code_column"]
----
include::hotspots/SystemResource.java[]
----
...
The container was deployed successfully, however it is isolated and can not be accessed for requests. A service needs to be created to expose your deployment so that you can make requests to your container. Update your [hotspot file=0]`deploy.yaml` file to include a service resource. 
...
To update your resources, simply rerun the following command:
...
Now you can access your microservice through the host name provided in the output. Curling just the host name will take you to the index page of your Open Liberty web application. The system properties is located in the path as outlined in the source code of the application. The [hotspot=path file=2]`@ApplicationPath` annotation denotes that all the resources for the system application can be found in the `/system/` path. The system resource is then placed at the `/properties` path, as denoted by the [hotspot=path file=3]`@Path` annotation, making the total path to the system properties application `[host-name]/system/properties` To learn more about how to use JAX-RS to create the system application, check out the https://openliberty.io/guides/rest-intro.html[Creating a RESTful web service] guide. Run the following `curl` command, ensuring that you replace the `[host-name]` with your appropriate host name:

[role=command]
```
curl [host-name]/system/properties
```
...
//TODO Deploying the second microservice
== Deploying the second microservice
...
To build and package the `inventory` microservice, the parent [hotspot file=0]`pom.xml` needs to be updated to add the `inventory` project to the list of modules to be built. Currently, it only specified the `system` project, so when the `mvn package` command was executed previously, it only built the `system` microservice. You can verify this by looking inside the `system` directory, and noticing that it contains the `target` directory that the `inventory` directory lacks.
...
----
[role="edit_command_text"]
Update the pom.xml file to add the [hotspot=inventory file=0]`inventory` module.

Ensure you are in the `start` directory, and run the following command to build and package the two `system` and `inventory` modules:
...
You can use the same [hotspot file=1]`deploy.yaml` configuration file to deploy multiple microservices. Update the configuration file to include the deployment, service, and route resources for the inventory microservice.
...
----
[role="edit_command_text"]
Update the configuration file to add the [hotspot=inventory file=0]`inventory` module, making sure to update the inventory [hotspot=inventoryImage file=1]`image` field with the appropriate image link.
...
Run the following `curl` command to view the current inventory, ensuring that you replace the `[host-name]` with your appropriate host name:

[role=command]
```
curl [host-name]/inventory/systems
```
...
Date: Fri, 08 Nov 2019 17:54:11 GMT
Set-Cookie: b5f6ba60ae9ca27f888447c48b36fb5d=2fba561e0e6ee559d9090efd5d93dace; path=/; HttpOnly
...
Note that since this is a fresh deployment, there are no saved systems in the inventory. Run the following command to allow the `inventory` microservice to access the `system` microservice and save the system result in the inventory:

[role=command]
```
curl [host-name]/inventory/systems/system-service
```

You will receive your JVM system properties as a response, now go ahead and run the following command to recheck the inventory:
...
Content-Length: 108
Content-Type: application/json
Date: Fri, 08 Nov 2019 17:56:23 GMT
Set-Cookie: b5f6ba60ae9ca27f888447c48b36fb5d=2fba561e0e6ee559d9090efd5d93dace; path=/; HttpOnly
X-Powered-By: Servlet/4.0

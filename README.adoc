...
The container was deployed successfully, however it is isolated and can not be accessed for requests. A service needs to be created to expose your deployment so that you can make requests to your container. Update your [hotspot]`deploy.yaml` file to include a service resource. 
...
----
[role="edit_command_text"]
Update the configuration file to include the service resource.
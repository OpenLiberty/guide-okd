...
You can add the `--url` flag at the end of the `minishift console` command to only display the URL. Log in with the following credentials:
...
Now that the environment is set up, ensure that you are in the `start` directory and run the following command to build a Docker image according to the instructions in the provided [hotspot file=0]`Dockerfile`:
...
Access your microservice through the host name provided in the output, by visiting the following [https://[host-name]/system/properties](https://[host-name]/system/properties]), or running the following command, ensuring to replace `[host-name]` with the proper host name provided by the `oc get routes` command.
...
Visit the following link, [https://[host-name]/inventory/systems]([host-name]/inventory/systems) or run the following `curl` command to view the current inventory, ensuring that you replace the `[host-name]` with your appropriate host name:
...
You will receive your JVM system properties as a response. Run the following command to recheck the inventory:
